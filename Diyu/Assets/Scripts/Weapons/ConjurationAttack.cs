...
namespace Weapons
{
    public class ConjurationAttack : RangedWeapon
    {
        private readonly GameObject _elementball;
        ...
        public ConjurationAttack(Rarities rarity,Entity user)
        {
            Name = "Conjuration Catalyst";
            switch (rarity)
            {
            ...
            CurrentCooldown = 0;
            _elementball = User.resources.projectileList[4];
        ...
        public override void CmdAttack()
        {
            ...
            if (CanAttack)
            {
                ...
                syncManager.CmdSpawnFireball(4,position,(baseDamage + damagePercent * User.abilityPower),anchor.transform.forward);
            }
        }
        [ClientRpc]
        public override void AttackRpc()
        {
            timeSinceLastAttack = 0;
            var position = anchor.transform.position;
            GameObject newFireball = Object.Instantiate(_elementball, position, Quaternion.identity);
            newFireball.GetComponent<Elementball>().damage = (baseDamage + damagePercent * User.abilityPower);
            newFireball.GetComponent<Elementball>().attacker = User;
